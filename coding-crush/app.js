...
function createBoard() {
  for (let i = 0; i < width*width; i++) {
    ...
    let randomColor = Math.floor(Math.random() * codingColors.length)
    square.style.backgroundImage = codingColors[randomColor]
    ...
    for (i = 0; i < 55; i ++) {
        if(squares[i + width].style.backgroundImage === '') {
            ...
            if (isFirstRow && (squares[i].style.backgroundImage === '')) {
              let randomColor = Math.floor(Math.random() * codingColors.length)
              squares[i].style.backgroundImage = codingColors[randomColor]
            }
        }
    }
}
